...
namespace ComputedMath.MathExtensions {
    ...
    public class CubicSpline {
        ...
        private double EvalSpline(double x, int j, bool debug = false) {
            ...
            double y = (1 - t) * _yOrig[j] + t * _yOrig[j + 1] +
                       t * (1 - t) * (_a[j] * (1 - t) + _b[j] * t); // equation 9
            return y;
        }
        ...
        public double[] EvalSlope(double[] x, bool debug = false) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                // Evaluate using j'th spline
                double dx = _xOrig[j + 1] - _xOrig[j];
                double dy = _yOrig[j + 1] - _yOrig[j];
                double t = (x[i] - _xOrig[j]) / dx;
        ...
        public static void FitParametric(double[] x, double[] y, int nOutputPoints, out double[] xs, out double[] ys,
            double firstDx = double.NaN, double firstDy = double.NaN, double lastDx = double.NaN,
