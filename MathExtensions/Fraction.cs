...
namespace ComputedMath.MathExtensions {
    ...
    /// </summary>
    public class Fraction : ILaTexConvertable, IComparable<Fraction> {
        ...
        public string ToLaTeX() {
            ...
            return Denominator == 1
                ? $"{{{Numerator.ToString()}}}"
                : $"{{ {sign} \\dfrac{{ {Math.Abs(Numerator)} }}{{ {Denominator} }} }}";
        }
        ...
        public Fraction Absolute() {
            return Numerator < 0 ? new Fraction(-Numerator, Denominator) : this;
        }
